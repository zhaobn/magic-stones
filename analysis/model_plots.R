...
# Prep data for plot
ppt<-df.freq%>%mutate(type='behavioral')%>%select(learningTaskId, trial, type, selection, value=freq)
rev<-df.rev%>%mutate(type='reverse')%>%select(learningTaskId, trial, type, selection, value=freq)
nm<-df.pred%>%mutate(type='normative') %>%select(learningTaskId, trial, type, selection, value=pp)
dd<-df.sim%>%mutate(type='data_driven') %>%select(learningTaskId, trial, type, selection, value=prob)
wg<-df.weighted%>%mutate(type='weighted') %>%select(learningTaskId, trial, type, selection, value=prob)
w2<-df.w2%>%mutate(type='w2') %>%select(learningTaskId, trial, type, selection, value=prob)
